import {
  CodeExampleSet,
  CodeLanguage,
  OperationType,
  Step,
} from "../types";

interface CodeExamplesPanelProps {
  step?: Step;
  fallbackOperation: OperationType;
  selectedLanguage: CodeLanguage;
  onLanguageChange: (language: CodeLanguage) => void;
  codeExamples: CodeExampleSet;
  title: string;
}

export function CodeExamplesPanel({
  step,
  fallbackOperation,
  selectedLanguage,
  onLanguageChange,
  codeExamples,
  title,
}: CodeExamplesPanelProps) {
  const operation = step?.operation ?? fallbackOperation;
  const snippet = codeExamples.snippets[operation][selectedLanguage];
  const fullImplementation = codeExamples.fullImplementations[selectedLanguage];
  const stepNote = step
    ? codeExamples.modeNotes[step.highlights.mode] ??
      "このステップでは、可視化されている動きとコード内のループや分岐を対応づけて見ます。"
    : "短いスニペットで処理の骨格を掴み、必要なら完全実装を開いて全体像を確認します。";

  return (
    <section className="panel panel-code">
      <div className="panel-header">
        <p className="eyebrow">Code Examples</p>
        <h2>{title}</h2>
        <p className="panel-copy">
          {operation.toUpperCase()} に対応する実装を、可視化のすぐ下で読めます。
        </p>
      </div>

      <div className="language-toggle" role="tablist" aria-label="Code language">
        {(["python", "c"] as const).map((language) => (
          <button
            key={language}
            type="button"
            className={
              language === selectedLanguage
                ? "language-button is-selected"
                : "language-button"
            }
            onClick={() => onLanguageChange(language)}
          >
            {language === "python" ? "Python" : "C"}
          </button>
        ))}
      </div>

      <div className="code-card">
        <p className="detail-label">Snippet</p>
        <strong>{snippet.title}</strong>
        <span>{stepNote}</span>
        <pre className="code-block">
          <code>{snippet.code}</code>
        </pre>
      </div>

      <details className="code-details">
        <summary>Complete implementation</summary>
        <div className="code-card code-card-full">
          <strong>{fullImplementation.title}</strong>
          <span>
            短いスニペットで見た処理が、全体のどこにあるかを確認するための最小実装です。
          </span>
          <pre className="code-block">
            <code>{fullImplementation.code}</code>
          </pre>
        </div>
      </details>
    </section>
  );
}
